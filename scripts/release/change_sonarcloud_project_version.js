...
const SONAR_PROJECT_PROPERTIES_FILE = path.join(__dirname, "../../sonar-project.properties")
...
const properties = new PropertiesEditor(readFileSync(SONAR_PROJECT_PROPERTIES_FILE, "utf8"))
...
try {
  ...
  console.log(`Updating ${SONAR_PROJECT_PROPERTIES_FILE} to version ${newVersion}`)
} catch (error) {
  console.error(error)
  process.exit(1)
}
