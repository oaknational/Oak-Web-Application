...
in the next-sitemap.config.js file (not without duplicating some Next routing functionality). Instead we create server side sitemaps on static routes that specify all the dynamic path pages under that route.

**Pages Router** sitemaps use `getServerSideSitemapLegacy` from `next-sitemap`, e.g. blog and webinar sitemaps under `src/pages/`.

**App Router** sitemaps export a default function returning `MetadataRoute.Sitemap`, e.g. the teachers sitemap at [src/app/teachers/sitemap.ts](../src/app/teachers/sitemap.ts) (`/teachers/sitemap.xml`).
