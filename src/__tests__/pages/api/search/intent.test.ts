...
const mockWithSuggestionCache = jest.fn(
  (_searchTerm: string, fn: () => Promise<unknown>) => fn(),
);
jest.mock("@/context/Search/ai/withSuggestionCache", () => ({
  withSuggestionCache: (
    searchTerm: string,
    fn: () => Promise<unknown>,
  ): Promise<unknown> => mockWithSuggestionCache(searchTerm, fn),
}));

const mockGenerateSuggestions = jest.fn();
jest.mock("@/context/Search/ai/generateSuggestions", () => ({
  generateSuggestions: (searchTerm: string) =>
    mockGenerateSuggestions(searchTerm),
}));
...
describe("/api/search/intent", () => {
  beforeEach(() => {
    ...
    mockWithSuggestionCache.mockImplementation(
      (_searchTerm: string, fn: () => Promise<unknown>) => fn(),
    );
  ...
  describe("direct matches", () => {
    ...
    it("should not call AI or cache when there is a direct subject match", async () => {
      const { req, res } = createNextApiMocks({
        method: "GET",
      ...
      expect(mockWithSuggestionCache).not.toHaveBeenCalled();
    });
  });

  describe("AI suggestions", () => {
    it("should call AI and return suggestions when there is no direct subject match", async () => {
      mockGenerateSuggestions.mockResolvedValue([
      ...
      expect(mockGenerateSuggestions).toHaveBeenCalledWith("golf");
      expect(mockWithSuggestionCache).toHaveBeenCalledWith(
        "golf",
        expect.any(Function),
      );
      expect(res._getJSONData()).toEqual({
    ...
    it("should call AI when there is a partial match without subject", async () => {
      mockGenerateSuggestions.mockResolvedValue([
        { slug: "english", confidence: 4 },
      ]);

      const { req, res } = createNextApiMocks({
        method: "GET",
        query: { searchTerm: "ks3 shakespeare" },
      });
      ...
      expect(res._getStatusCode()).toBe(200);
      expect(mockGenerateSuggestions).toHaveBeenCalledTimes(1);
      expect(mockGenerateSuggestions).toHaveBeenCalledWith("ks3 shakespeare");
    });
