...
import { getServerSideProps as getServerSidePropsSitemap } from "@/pages/webinars/categories/sitemap.xml";

const blogPostsMock = jest.fn();
jest.mock("@/node-lib/cms", () => ({
  __esModule: true,
  default: {
    webinars: () => blogPostsMock(),
  },
}));
  ...
  describe("getServerSideProps", () => {
    it("fetches data and generates fields correctly - sitemap.xml ", async () => {
      ...
      for (const expectedString of [
        "http://example.com/webinars/categories/foo",
        "http://example.com/webinars/categories/bar",
        "http://example.com/webinars/categories/baz",
      ]) {
