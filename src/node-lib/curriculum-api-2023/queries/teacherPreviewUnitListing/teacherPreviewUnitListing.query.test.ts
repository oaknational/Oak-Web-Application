...
import teachersPreviewUnitListing from "./teacherPreviewUnitListing.query";

import sdk from "@/node-lib/curriculum-api-2023/sdk";
import { UnitSnake } from "@/node-lib/curriculum-api-2023/queries/unitListing/unitListing.schema";
...
jest.mock("../../sdk", () => {
  return {
    ...
    teachersPreviewUnitListing: jest.fn(() =>
      Promise.resolve({
        units: [
          rawFixture({
            overrides: {
...
describe("teachersPreviewUnitListing()", () => {
  test("returns null if no unit is found", async () => {
    const res = await teachersPreviewUnitListing({
      ...sdk,
      teachersPreviewUnitListing: jest.fn(() => Promise.resolve({ units: [] })),
    })({
  ...
  test("returns the correct data", async () => {
    const res = await teachersPreviewUnitListing(sdk)({
    ...
    expect(res).toEqual({
      ...
      tierSlug: "foundation",
      phase: "primary",
      yearGroups: [{ yearTitle: "Year 1", yearSlug: "year-1" }],
      subjectCategories: [
        {
          label: "Physics",
