...
export const nationalCurriculumInsightsModuleSchema = z.discriminatedUnion(
  ...
  [
    ...
    nationalCurriculumInsightsGuidanceIntroSectionSchema,
...
export const nationalCurriculumInsightsHubSchema = z
  ...
  .superRefine(({ subjects }, context) => {
    ...
    if (subjectSlugs.includes("guidance")) {
      context.addIssue({
        code: "custom",
        path: ["subjects"],
        message:
          "The guidance slug is reserved for the practical guidance page",
      });
    }
...
export const nationalCurriculumInsightsGuidancePageSchema = z.object({
  pageType: z.literal("guidance"),
  title: z.string().min(1),
  summary: z.string().min(1),
  modules: z.array(nationalCurriculumInsightsModuleSchema).min(1),
  ...documentSchema.shape,
...
>;
export type NationalCurriculumInsightsGuidancePage = z.infer<
  typeof nationalCurriculumInsightsGuidancePageSchema
>;
