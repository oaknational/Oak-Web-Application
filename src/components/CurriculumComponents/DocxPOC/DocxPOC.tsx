...
Comparing quantities - part whole relationships`,
  ...
  const handleDownloadClick = () => {
    ...
    reader.onload = function (event) {
      ...
      // Patch the template document. The upper case keys are the placeholders in the template
      patchDocument(uint8Array, {
        patches: {
          SUBJECT: {
            type: PatchType.PARAGRAPH,
            children: [new TextRun(formData.subject)],
          },
          CURRICULUM_EXPLAINER: {
            type: PatchType.PARAGRAPH,
            children: [new TextRun(formData.curriculum_explainer)],
          },
          LINK_TO_NEW_UNIT_TABLE: {
            ...
            children: [
              new InternalHyperlink({
                children: [
                  new TextRun({
                    text: formData.link_to_new_table,
                    style: "Hyperlink",
                  }),
                ],
          ...
          SUBJECT_PRINCIPLES_LIST: {
            // Added a new patch type for JSON, which allows for more complex structures
            type: PatchType.JSON,
            children: subjectPrinciples(formData.principles.split("\n")),
          },
          UNIT_LIST_TABLE: {
            type: PatchType.JSON,
