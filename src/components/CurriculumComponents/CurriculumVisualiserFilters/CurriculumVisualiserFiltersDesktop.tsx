import { OakP, OakRadioButton, OakRadioGroup, OakSpan, OakSmallSecondaryButton, OakFlex, OakBox, OakHeading } from "@oaknational/oak-components";
...
import { CurriculumFilters, CurriculumVisualiserFiltersProps } from "./CurriculumVisualiserFilters";
import { highlightedUnitCount } from "./helpers";

import Box from "@/components/SharedComponents/Box";
import { getYearGroupTitle } from "@/utils/curriculum/formatting";
import { Thread, Subject, SubjectCategory, Tier } from "@/utils/curriculum/types";
import { CurriculumUnitsFormattedData } from "@/pages-helpers/curriculum/docx/tab-helpers";

function getFilterData (yearData: CurriculumUnitsFormattedData["yearData"]) {
  const units = Object.entries(yearData).flatMap(([year, units]) => units)
  const childSubjects = new Map<string, Subject>();
  const subjectCategories = new Map<number, SubjectCategory>();
  const tiers = new Map<string, Tier>();
  units.forEach(u => {
    u.childSubjects.forEach(childSubject => childSubjects.set(childSubject.subject_slug, childSubject))
    u.tiers.forEach(tier => tiers.set(tier.tier_slug, tier))
    u.subjectCategories.forEach(subjectCategory => subjectCategories.set(subjectCategory.id, subjectCategory))
  })
...
export default function CurriculumVisualiserFiltersDesktop({
  ...
  const {childSubjects, subjectCategories, tiers} = getFilterData(data.yearData)
  ...
  return (
    <OakBox $mr={"space-between-s"}>
      ...
      <OakHeading tag="h4" $mb="space-between-xs">Year group</OakHeading>
      <OakFlex $flexWrap={"wrap"} $gap={"all-spacing-1"}>
        ...
        >All</OakSmallSecondaryButton>
        {yearOptions.map((yearOption) => {
          return <OakSmallSecondaryButton
            iconName={filterIncludes("years", [yearOption]) ? "tick" : undefined}
            onClick={() => toggleInFilter("years", yearOption)}
          >{getYearGroupTitle(yearData, yearOption)}</OakSmallSecondaryButton>
        })}
      </OakFlex>
        ...
        <OakBox>
          ...
          <OakFlex $flexWrap={"wrap"} $gap={"all-spacing-1"}>
            {subjectCategories.map((subjectCategory) => {
              return <OakSmallSecondaryButton
                iconName={filterIncludes("subjectCategories", [String(subjectCategory.id)]) ? "tick" : undefined}
                onClick={() => setSingleInFilter("subjectCategories", String(subjectCategory.id))}
              >{subjectCategory.title}</OakSmallSecondaryButton>
            })}
          </OakFlex>
        </OakBox>
      }
        
      {tiers.length > 0 && <OakBox>
        <OakHeading tag="h4" $mb="space-between-xs" $mt="space-between-m2">
          Learning tier (KS4)
        </OakHeading>
        <OakFlex $flexWrap={"wrap"} $gap={"all-spacing-1"}>
          {tiers.map((tier) => {
            return <OakSmallSecondaryButton
              iconName={filterIncludes("tiers", [tier.tier_slug]) ? "tick" : undefined}
              onClick={() => setSingleInFilter("tiers", tier.tier_slug)}
            >{tier.tier}</OakSmallSecondaryButton>
          })}
        </OakFlex>
      </OakBox>}
        
      {childSubjects.length > 0 && <OakBox>
        <OakHeading tag="h4" $mb="space-between-xs" $mt="space-between-m2">
          Exam subject (KS4)
        </OakHeading>
        <OakFlex $flexWrap={"wrap"} $gap={"all-spacing-1"}>
          {childSubjects.map((childSubject) => {
            return <OakSmallSecondaryButton
              iconName={filterIncludes("childSubjects", [childSubject.subject_slug]) ? "tick" : undefined}
              onClick={() => setSingleInFilter("childSubjects", childSubject.subject_slug)}
            >{childSubject.subject}</OakSmallSecondaryButton>
          })}
        </OakFlex>
      </OakBox>}

      <FieldsetLegend $font={"heading-7"} $mb="space-between-xs" $mt="space-between-m2">
        Threads
      </FieldsetLegend>
      <RadioGroup
        name="thread"
        onChange={(e) => onChangeFilters({...filters, "threads": [e.target.value]})}
