    ...
    getProps: async () => {
      ...
      if (ENABLE_OPEN_API) {
        const res = await fetch(
          `${process.env.OPENAPI_URL}/api/v0/sequences/${context.params.subjectPhaseSlug}/units`,
          {
            headers: {
        ...
        curriculumUnitsTabData = {
          units: apiUnits.map<CombinedCurriculumData["units"][number]>(
            (apiUnit) => {
              const subject = apiUnit.subject
                ? apiUnit.subject
                : slugs.subjectSlug;
