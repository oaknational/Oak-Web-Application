 ...
 */
import LessonDownloadsSuccessVariantPage, { generateMetadata } from "./page";
...
describe("LessonDownloadsSuccessVariantPage", () => {
  beforeEach(() => {
    mockTeachersUnitOverview.mockResolvedValue(unitFixture);
  });

  it("fetches unit data and renders the compact success confirmation", async () => {
    const result = await LessonDownloadsSuccessVariantPage({
  ...
  it("renders 404 when lesson release date is missing", async () => {
    ...
    await expect(
      LessonDownloadsSuccessVariantPage({
        params: Promise.resolve(defaultParams),
        searchParams: Promise.resolve({}),
      }),
...
describe("generateMetadata", () => {
  ...
  it("returns empty object when lesson release date is missing", async () => {
    mockTeachersUnitOverview.mockResolvedValue({
      ...unitFixture,
      lessons: [
        {
